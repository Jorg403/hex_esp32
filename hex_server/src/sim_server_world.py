...
World-frame rendering:
  ...
  • The body hexagon translates as the robot walks.
  • A fading trail marks where the body has been.
  • A bounding box shows the total travel extent.
  • Camera is centred on the current stance-foot centroid (soft follow).
...
# Ordered list of body→leg transform matrices
_TM = [
...
class SimState:
    """Thread-safe simulation state with world-frame foot odometry."""

    def __init__(self):
        self.pos_body  = consts.INITIAL_POSITIONS_BODY.copy()   # (6,3) body frame
        self.pos_leg   = consts.INITIAL_POSITIONS.copy()        # (6,3) leg frame

        # World-frame tracking — initialise feet at their body-frame start positions
        # (body starts at world origin, so body-frame == world-frame initially).
        self.foot_world = consts.INITIAL_POSITIONS_BODY[:, :2].copy().astype(np.float32)  # (6,2)
        self.body_world = np.zeros(2, dtype=np.float32)                                   # (2,)
        self.is_stance  = np.ones(6, dtype=bool)
        self.body_trail: list = []        # list of np.ndarray (2,)
        self.cam_pos    = np.zeros(2, dtype=np.float32)   # smoothed camera world pos
    ...
    def update(self, angles_flat: list) -> None:
        ...
        foot_body2  = pos_body[:, :2]          # (6,2) XY in body frame

        with self.lock:
            foot_world = self.foot_world.copy()
            prev_body  = self.body_world.copy()

        # --- Odometry: body world pos from stance feet ---
        # For a stance foot:  foot_world_fixed == body_world + foot_body_xy
        # Therefore:          body_world = foot_world_fixed - foot_body_xy
        stance_idx = [i for i in range(6) if is_stance[i]]
        if stance_idx:
            estimates  = np.stack([foot_world[i] - foot_body2[i] for i in stance_idx])
            body_world = estimates.mean(axis=0).astype(np.float32)
        else:
            body_world = prev_body.copy()

        # --- Update swing foot world positions ---
        new_foot_world = foot_world.copy()
        for i in range(6):
            if not is_stance[i]:
                new_foot_world[i] = body_world + foot_body2[i]

        # --- Camera target: centroid of current stance feet ---
        # While feet are in stance their world positions are fixed, so the centroid
        # is fixed too — the camera stays still and the body hexagon moves.
        if stance_idx:
            cam_target = np.stack([new_foot_world[i] for i in stance_idx]).mean(axis=0).astype(np.float32)
        else:
...
def w2px(wx: float, wy: float, cam: np.ndarray):
    """Convert world (cm) to pixel coords in the left panel."""
...
def draw_grid(img: np.ndarray, cam: np.ndarray) -> None:
    ...
    sp = 5.0 * WORLD_SCALE          # pixels per cell

    # Phase: where the world-origin grid line falls in screen space
...
def draw_trail(img: np.ndarray, trail: list, cam: np.ndarray) -> None:
    """Fading body-centre trail (blue → bright blue = older → newer)."""
    n = len(trail)
    if n < 2:
        return
    for i in range(1, n):
        alpha = i / n
        v     = int(40 + 180 * alpha)
        col   = (v, v // 2, v // 4)    # fades: dark amber → bright amber-white
...
def draw_bounding_box(img: np.ndarray, trail: list, cam: np.ndarray) -> None:
    """Dashed-style bounding box around the body's total travel extent."""
    if len(trail) < 4:
        return
    xs  = [p[0] for p in trail]
    ys  = [p[1] for p in trail]
    pad = 4.0   # cm padding around extent
    ...
    # Size label above top-left corner
    dx  = max(xs) - min(xs)
    dy  = max(ys) - min(ys)
    lbl = f"{dx:.1f} x {dy:.1f} cm"
    cv2.putText(img, lbl, (tl[0] + 3, tl[1] - 5),
                cv2.FONT_HERSHEY_SIMPLEX, 0.34, C_BBOX, 1)


def draw_body(img: np.ndarray, body_world: np.ndarray, cam: np.ndarray) -> None:
    """Body hexagon at its current world position, with a forward-direction arrow."""
    bx, by = float(body_world[0]), float(body_world[1])
    # Leg-mount points: body origin + TM translation offset (no heading rotation tracked)
    pts = np.array(
        [w2px(bx + tm[0, 3], by + tm[1, 3], cam) for tm in _TM],
        dtype=np.int32,
    )
    cv2.polylines(img, [pts], isClosed=True, color=C_BODY, thickness=2)
    centre = w2px(bx, by, cam)
    fwd    = w2px(bx + 4.0, by, cam)
...
def draw_legs(img: np.ndarray, foot_world: np.ndarray,
              body_world: np.ndarray, is_stance: np.ndarray,
              cam: np.ndarray) -> None:
    """Leg lines and foot dots in world frame.
    Stance feet stay visually fixed; swing feet move."""
    bx, by = float(body_world[0]), float(body_world[1])
    for i in range(6):
        mount = w2px(bx + _TM[i][0, 3], by + _TM[i][1, 3], cam)
...
# ── Right-panel helpers (unchanged logic) ─────────────────────────────────────
...
def draw_panel(img: np.ndarray, state: SimState) -> None:
    ...
    y += 4
    txt("── WORLD POS ──", C_HIGHLIGHT, bold=True)
...
def build_frame(state: SimState) -> np.ndarray:
    ...
    # ── Left panel: world-frame top-down view ──────────────────────────────
    draw_grid(img, cam)
    draw_trail(img, trail, cam)
    draw_bounding_box(img, trail, cam)
    draw_legs(img, foot_world, body_world, is_stance, cam)
    draw_body(img, body_world, cam)

    # Title
    cv2.putText(img, "Hexapod Sim  — World Frame", (10, 22),
                cv2.FONT_HERSHEY_SIMPLEX, 0.55, C_HIGHLIGHT, 2)
    bpos = f"body ({body_world[0]:+.1f}, {body_world[1]:+.1f}) cm   5 cm grid"
    cv2.putText(img, bpos, (10, 42),
                cv2.FONT_HERSHEY_SIMPLEX, 0.37, C_DIM, 1)

    # ── Right panel ────────────────────────────────────────────────────────
    # Blank out anything that leaked past the divider
